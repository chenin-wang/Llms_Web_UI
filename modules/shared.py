...
# 用于存放ui中的定义的变量所对应的gradio中的类型，slider, checkbox等
input_elements=[] 
...
parser.add_argument("--model-dir", type=str, default='/usr/models/', help="Path to directory with all the models")
...
with Path(f'{args.model_dir}/config.yaml') as p:
    if p.exists():
        model_config = yaml.safe_load(open(p, 'r').read()) # 模型config.yaml中读取的配置信息
    else:
        model_config = {}
