...
def create_ui(default_preset):
    ...
    with gr.Tab("Parameters", elem_id="parameters"):
        ...
        ### Temperature
        Primary factor to control randomness of outputs. range: [0.01, 1]. 0 = deterministic (only the most likely token is used). Higher value = more randomness.
        ### top_p
        If not set to 1, select tokens with probabilities adding up to less than this number. Higher value = higher range of possible random results.
        核心采样（nucleus sampling），是另一个控制语言模型输出随机性的超参数配置。它设定了一个概率阈值，并选择累积概率超过该阈值的最佳词汇，然后模型从这组词汇中随机
        抽取以生成输出。与传统方法（在整个词汇表中随机抽样）相比，这种方法可以产生更丰富多样且有趣的输出。例如 top p 为 0.9 时模型将仅考虑概率阈值 90% 以上的词汇。
        ### top_k
...
def create_event_handlers():
    ...
    shared.gradio['preset_menu'].change(presets.load_preset_for_ui, gradio('preset_menu', 'interface_state'), gradio('interface_state') + gradio(presets.presets_params())).then(
        lambda x,y: (x,y), gradio('temperature','top_p'), gradio('langchain_temperature',"langchain_top_p")
    )
    shared.gradio['temperature'].change(lambda x: x, gradio('temperature'), gradio('langchain_temperature'))
    shared.gradio['top_p'].change(lambda x: x, gradio('top_p'), gradio('langchain_top_p'))
