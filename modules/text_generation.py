...
def generate_reply_HF(question, original_question, seed, state, stopping_strings=None, is_chat=False):
    generate_params = {}
    for k in ['max_new_tokens', 'do_sample', 'temperature', 'top_p', 'typical_p', 'repetition_penalty', 'repetition_penalty_range', 
    ...
    # logger.info(f"shared.tokenizer.eos_token_id:{shared.tokenizer.eos_token_id}")
    eos_token_ids = [shared.tokenizer.eos_token_id] if shared.tokenizer.eos_token_id is not None else []
    generate_params['eos_token_id'] = eos_token_ids
    # logger.info(f"eos_token_id:{generate_params['eos_token_id']}")
    ...
    try:
        ...
        # This is based on the trick of using 'stopping_criteria' to create an iterator.
            ...
            def generate_with_callback(callback=None, *args, **kwargs):
                ...
                clear_torch_cache()
                with torch.no_grad(): # 此处generate使用了 @torch.no_grad()
                    shared.model.generate(**kwargs) # 根据传入的参数进行生成答案
