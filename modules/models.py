...
def huggingface_loader(model_name):
    path_to_model = Path(f'{shared.args.model_dir}/{model_name}')
    if 'chatglm' in model_name.lower(): #清华智谱的chatglm网络结构特殊
        LoaderClass = AutoModel
    else:
        config = AutoConfig.from_pretrained(path_to_model, trust_remote_code=shared.args.trust_remote_code)
        if config.to_dict().get("is_encoder_decoder", False):
            LoaderClass = AutoModelForSeq2SeqLM # with a sequence-to-sequence language modeling head
            shared.is_seq2seq = True
        else:
            LoaderClass = AutoModelForCausalLM # with a causal language modeling head 
    ...
    if not any([shared.args.cpu, shared.args.load_in_8bit, shared.args.load_in_4bit, shared.args.auto_devices, 
        ...
        logger.info("Load the model in simple 16-bit mode by default")
        model = LoaderClass.from_pretrained(Path(f"{shared.args.model_dir}/{model_name}"), low_cpu_mem_usage=True, 
                                            torch_dtype=torch.bfloat16 if shared.args.bf16 else torch.float16, trust_remote_code=shared.args.trust_remote_code)
    ...
    # DeepSpeed ZeRO-3
        ...
        logger.info("Load the model in ZeRO-3 mode")
    ...
    # Custom
        ...
        if shared.args.load_in_8bit and params.get('max_memory', None) is not None and params['device_map'] == 'auto':
            ...
            with init_empty_weights():
                logger.info(f"Loading model in 8bit precision, the config {config}")
        ...
        logger.info(f"Loading model of params {params}")
